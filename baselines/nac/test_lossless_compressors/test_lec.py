...
OUTPUT_COLUMNS = utils.TEST_COMPRESSION_COLUMN_NAMES + ["block_size", "n_codebooks", "gpu", "k", "overlap"]
    ...
    def evaluate(path: str):
        ...
        with torch.no_grad():
            ...
            bottleneck = lec.encode(
                waveform = waveform, sample_rate = sample_rate, model = model, device = device, block_size = args.block_size, k = args.rice_parameter, overlap = args.overlap,
                log_for_zach_kwargs = {"duration": duration_audio, "lossless_compressor": "lec", "parameters": {"block_size": args.block_size, "n_codebooks": args.n_codebooks, "gpu": using_gpu, "k": args.rice_parameter, "overlap": args.overlap}, "path": path}, # arguments to log for zach
            ) # compute compressed bottleneck
            duration_encoding = time.perf_counter() - start_time # measure speed of compression
            round_trip = lec.decode(bottleneck = bottleneck, model = model, device = device, k = args.rice_parameter, overlap = args.overlap) # reconstruct waveform from bottleneck to ensure losslessness
        ...
        pd.DataFrame(data = [dict(zip(
            OUTPUT_COLUMNS, 
            (path, size_original, size_compressed, compression_rate, duration_audio, duration_encoding, compression_speed, args.block_size, args.n_codebooks, using_gpu, args.rice_parameter, args.overlap)
